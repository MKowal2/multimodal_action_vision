...
class InceptionI3D(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True, final_endpoint='logits',
        ...
        self.layers[end_point] = Unit3D(self._in_channels, 64, kernel_size=[7, 7, 7], stride=(2, 2, 2), padding=3, name=name+end_point)
        ...
        end_point = 'AvgPool_5'
        self.layers[end_point] = nn.AvgPool3d(kernel_size=[8, 4, 4], stride=(1, 1, 1))
    ...
    def replace_logits(self, num_classes, device='cuda:0'):
        ...
        # Move to GPU
        if 'cuda' in device: self.logits.cuda(device)

    def replace_input_layer(self, num_in_channels, device='cuda:0'):
        if num_in_channels < 1:
            raise ValueError('Number of input channels needs to be greater than 0!')
        self.in_channels = num_in_channels

        self.layers['Conv3d_1a_7x7'] = Unit3D(
            in_channels=num_in_channels, output_channels=64,
            kernel_size=[7, 7, 7], stride=(2, 2, 2), padding=3, name=self._model_name+'Conv3d_1a_7x7')

        self.Conv3d_1a_7x7 = self.layers['Conv3d_1a_7x7']

        # Weight initialization for new logits layer
        self._init_weights(self.layers['Conv3d_1a_7x7'].modules())

        # Move to GPU
        if 'cuda' in device: self.layers['Conv3d_1a_7x7'].cuda(device)
